## InkSpirit v1.0.0-rc1 — First v1.0 Release Candidate（预发布）

v1.0 系列的第一个预发布版。延续 0.9.3 的稳定性目标：任何用户安装后，第一次启动必须看到砚灵，并且所有失败都有可恢复路径。

### 本轮新增

- **启动链加固**：数据库访问不再发生在模块加载期（数据库损坏也能进入恢复流程，而不是在启动第一步直接崩溃）；窗口装饰/几何调用失败不再阻断内容加载；渲染层首屏报错显示可恢复界面（重新加载按钮），不再出现永久空白窗口
- **自定义头像**：在「我的砚灵 → 生命状态」里可以为它设置自己的头像（PNG/JPG/GIF/WebP，≤20MB），随备份一起导出/恢复；头像文件丢失时自动回退默认首字头像
- **窗口交互修复**：设置页可以拖动窗口（顶部标题区 + 左侧导航栏）；点击头像即可回到桌宠模式；关闭面板时不再在桌宠位置闪现窗口（先切透明视图再改窗口几何）；拖拽区双击不再意外最大化
- **文字显示修复**：AI 大脑页测试连接成功后不再重复显示两行「已连接/连接成功」文案

### 已知限制

- 预发布版：仅 Windows x64 安装包；Linux/macOS 目标后续版本补充
- 首次安装 v1.0.0-rc1 的数据库沿用 0.9.x 结构，schema 版本不变（8），升级安全
